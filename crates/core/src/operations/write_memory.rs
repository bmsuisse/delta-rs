...
/// Execution plan for reading in-memory batches of data
pub (crate) struct LazyPartitionMemoryExec {
    /// The partitions to query
    data: Box<dyn Iterator<Item = RecordBatch>>,
...
impl fmt::Debug for LazyPartitionMemoryExec {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "partition_columns: {:?}", self.partition_columns)?;
...
impl DisplayAs for LazyPartitionMemoryExec {
    fn fmt_as(
        &self,
        t: DisplayFormatType,
        ...
        match t {
            DisplayFormatType::Default | DisplayFormatType::Verbose => {
                ...
                write!(
                    f,
                    "LazyPartitionMemoryExec: partitions={}, partition_sizes={partition_sizes:?}{output_ordering}",
                    partition_sizes.len(),
                )
            }
        }
...
impl ExecutionPlan for LazyPartitionMemoryExec {
    ...
    fn output_partitioning(&self) -> Partitioning {
        Partitioning::UnknownPartitioning(self.partition_columns.len())
    }
    ...
    fn with_new_children(
        ...
        // LazyPartitionMemoryExec has no children
    ...
    fn execute(
        ...
        Ok(Box::pin(LazyMemoryStream::try_new(
            self.partition_columns[partition].clone(),
            self.projected_schema.clone(),
